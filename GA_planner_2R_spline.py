...
from viewer import LivePlotNotebook
from prm import PRM
from cell_decomposition import CellDecomposition
from post_process import ferguson_spline
import time
...
class GeneticAlgorithm():
    ...
    def __init__(self, world=None, NGEN=1000, n_ind=100, n_elite=10,
                 fitness_thresh=0.1, margin=False, verbose=True):
    ...
    def oneptcx(self, ind, pop):
        ...
        pop2 = [ind for ind in pop if len(ind1) == len(
            ind) and np.linalg.norm(ind - ind1) > 1e-8]
        pop = pop1 + pop2
...
if __name__ == '__main__':
    ...
    ga = GeneticAlgorithm(world=cspace, NGEN=10, n_ind=100, n_elite=10,
                          fitness_thresh=3.4, verbose=True)
    ...
    # Visualize
    best = ferguson_spline(ga.best_ind, 10)
    configs = [robot2R.forward_kinematics(pt) for pt in np.array(best)]
